...
+ (void)creatBaseDataTable
{
    ...
    if ([db open])
    {
        ...
        {
            [db executeUpdate:@"CREATE TABLE baseData (ProjectInfoId text,ProjectName text,projectSchedule text,Longitude text,Latitude text,StoreId text,StoreName  text,Code text,Address text)"];
        }
    }
    [db close];
}
...
+ (void)keepDataWithDictionary:(NSMutableDictionary *)dic
{
    ...
    if ([db open])
    {
        [db executeUpdate:@"insert into baseData (ProjectInfoId,ProjectName,projectSchedule,Longitude,Latitude,StoreName,Code,StoreId,Address) values(?,?,?,?,?,?,?,?,?)",[dic objectForKey:@"ProjectInfoId"],[dic objectForKey:@"ProjectName"],[dic objectForKey:@"projectSchedule"],[dic objectForKey:@"Longitude"],[dic objectForKey:@"Latitude"],[dic objectForKey:@"StoreName"],[dic objectForKey:@"Code"],[dic objectForKey:@"StoreId"],[dic objectForKey:@"Address"]];
    }
    [db close];
}
...
//            return [set stringForColumn:@"Code"];
            dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:[set stringForColumn:@"StoreId"],@"storeCode",[set stringForColumn:@"ProjectInfoId"],@"projectCode",[set stringForColumn:@"Code"],@"code",[set stringForColumn:@"Address"],@"Address", nil];
//            NSLog(@"%@",dic);
        }
        [set close];
    }
...
+ (void)UpdateAllBaseDataWithDictionary:(NSDictionary *)baseDataDic
{
    FMDatabase * db = [self getDataBaseDB];
    if ([db open]) {
...
//基础所有数据保存到数据库
+ (void)keepAllBaseDataWithDictionary:(NSDictionary *)baseDataDic
{
    
    
    [self keepDataWithDictionary:baseDataDic];
